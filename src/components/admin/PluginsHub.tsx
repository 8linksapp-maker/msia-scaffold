                ...
                {registry.map(p => {
                    ...
                    return (
                        <a
                            key={p.name}
                            href={p.href}
                            className="bg-surface p-6 rounded-lg border border-border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex flex-col gap-4 group"
                        >
                            <div className="flex items-start justify-between">
                                <div className={`w-11 h-11 rounded-md ${p.bg} flex items-center justify-center shrink-0`}>
                                    <Icon className={`w-5 h-5 ${p.color}`} />
                                </div>
                                <ChevronRight className="w-4 h-4 text-ink-faint group-hover:text-ink-muted group-hover:translate-x-0.5 transition-all mt-1" />
                            </div>
                            <div>
                                <h3 className="font-bold text-ink mb-1">{p.label}</h3>
